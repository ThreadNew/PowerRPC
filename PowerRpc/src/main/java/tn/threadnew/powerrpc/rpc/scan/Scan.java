package tn.threadnew.powerrpc.rpc.scan;
 ...
 */
public class Scan {
    public final static Scan me = new Scan();
    private List<Visitor> VISITORS = new LinkedList<>();
    private Scan(){
        VISITORS.add(new ClassVisitor());
    }
    public Scan accept(Visitor visitor) {
        VISITORS.add(visitor);
        return this;
    }

    // 扫描file
    private void scanPacket(File curFile, String packName) {
        //如果不是目录就结束方法的调用
        ...
        for (File file : files) {
            if (file.isFile() && file.getName().endsWith(".class")) {
                ...
                try {
                    ...
                    for (Visitor visitor : VISITORS) visitor.visitor(klass);
                ...
                //一直这样递归调用，直到是文件为止
                scanPacket(file, packName + "." + file.getName());
            }
        }
    }
